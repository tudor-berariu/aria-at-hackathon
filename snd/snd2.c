...
static int ochannel = 0;
static pthread_mutex_t ochannel_lock;

static void generate_sine(const snd_pcm_channel_area_t *areas,
              snd_pcm_uframes_t offset,
              int count, double *_phase)
{
    pthread_mutex_lock(&ochannel_lock);
    ...
    while (count-- > 0) {
        ...
        for (chn = 0; chn < channels; chn++) {
            for (byte = 0; byte < (unsigned int)bps; byte++)
                *(samples[chn] + byte) = (ochannel != chn) ? 0 : tmp[byte];
    ...
    *_phase = phase;
    pthread_mutex_unlock(&ochannel_lock);
}
...
pthread_t *start_sound_thread() {
   ...
   pthread_create(t, NULL, sound_thread, NULL);
